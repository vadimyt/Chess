...
if __name__ == '__main__':
    ...
    try:
        with Pool(processes=processes_count) as pool:
            ...
            server_res_array=([res.get() for res in multiple_results])
        print(server_res_array)
        first_complete = True
    except:
        print("First not complete")
    server_res_array_min = server_res_array[0][0]
    server_res_array_max = server_res_array[0][0]
    server_res_array_mid = 0
    for k in range(len(server_res_array)):
        for m in range(len(server_res_array)):
            server_res_array_mid+=server_res_array[k][m]
            if (server_res_array[k][m]>server_res_array_max):
                server_res_array_max=server_res_array[k][m]
            if (server_res_array[k][m]<server_res_array_min):
                server_res_array_min=server_res_array[k][m]
    ...
    for k in range(len(threaded_server_res_array)):
        for m in range(len(threaded_server_res_array)):
    ...
    if (first_complete and second_complete):
        print("server_res_array_min: " + str(server_res_array_min) + " threaded_server_res_array_min: " + str(threaded_server_res_array_min))
        print("server_res_array_max: " + str(server_res_array_max) + " threaded_server_res_array_max: " + str(threaded_server_res_array_max))
        print("server_res_array_mid: " + str(server_res_array_mid/len(server_res_array)) + " threaded_server_res_array_mid: " + str(threaded_server_res_array_mid/len(threaded_server_res_array)))
    else:
        if (first_complete):
            print("server_res_array_min: " + str(server_res_array_min))
            print("server_res_array_max: " + str(server_res_array_max))
            print("server_res_array_mid: " + str(server_res_array_mid/len(server_res_array)))
        else:
            if(second_complete):
                print("threaded_server_res_array: " + str(threaded_server_res_array_min))
                print("threaded_server_res_array: " + str(threaded_server_res_array_max))
                print("threaded_server_res_array: " + str(threaded_server_res_array_mid/len(threaded_server_res_array)))